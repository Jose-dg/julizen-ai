...
export function CheckoutForm() {
    const { totalPrice, isDiscountApplied, items } = useCart();
    ...
    const totalCents = isDiscountApplied ? 0 : totalCOP * 100;
    ...
    const handleWompiPayment = async () => {
        if (totalCents <= 0 && !isDiscountApplied) {
        ...
        const reference = isDiscountApplied ? `FREE-${Date.now()}` : `ORD-${Date.now()}`;

        // Save customer info and cart items to localStorage for the thanks page
        ...
        // Save cart items separately so they're available even if cart is cleared
        localStorage.setItem('order_items', JSON.stringify(items));

        if (isDiscountApplied) {
            window.location.href = `/thanks?id=${reference}&amount=0`;
            return;
        }
    ...
    return (
        <div className="max-w-xl mx-auto py-4 md:py-8 space-y-8">
            ...
            <Button
                ...
                disabled={loading || (totalCents <= 0 && !isDiscountApplied)}
